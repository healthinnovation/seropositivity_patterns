---
title: "Analisis_01"
format: html
editor: visual
---

## Análisis de FFI

### Tabla 1:

```{r}
knitr::opts_chunk$set(
  cache = TRUE,
  warnings = FALSE,
  messages = FALSE
)
```

Paso 1: Subir paquetes estadisticos + subir base de datos

```{r}
# Cargar los paquetes necesarios
library(tidyverse)     # Manipulación de datos
library(labelled)      # Etiquetas de variables (ideal para datos de REDCap)
library(gtsummary)     # Tablas descriptivas y comparativas
library(janitor)       # Tablas de frecuencias limpias
library(flextable)     # Exportar tablas bonitas a Word o HTML
library(readr)

```

Paso 2: Subir la base de datos

```{r}
ffi_hh_ind_seropos <- read_csv("./data/mid/ffi_hh_ind_seropos.csv") %>% 
  mutate(age_oms = case_when(
    age_cat %in% c("[0-10)", "[10-20)") ~ "Niño y adolescente",
    age_cat %in% c("[20-30)", "[30-40)") ~ "Adulto joven",
    age_cat %in% c("[40-50)", "[50-60)") ~ "Adulto medio",
    age_cat %in% c("[60-70)", "[70+)")   ~ "Adulto mayor",
    TRUE ~ NA_character_
  ),
  time_resi_cat2 = case_when(
  ffi_is_time_resi_years < 5 ~ "< 5 años",
  ffi_is_time_resi_years >= 5 & ffi_is_time_resi_years <= 20 ~ "5-20 años",
  ffi_is_time_resi_years > 20 ~ "> 20 años",
  TRUE ~ NA_character_
  ),
  education_summary = case_when(
    ffi_is_inst_level == 0 ~ "Sin escolaridad",
    ffi_is_inst_level %in% c(1, 2) ~ "Primaria",
    ffi_is_inst_level %in% c(3, 4) ~ "Secundaria",
    ffi_is_inst_level %in% c(5, 6) ~ "Educación superior",
    TRUE ~ NA_character_
  ),
  sympt_osteomuscular = if_else(
    ffi_is_symp_month_fatigue == 1 |
    ffi_is_symp_month_musclejoint == 1,
    "Sí", "No"
    ),
  Schistosomiasis = if_else(
      SEA_pos == 1 | X229.E.NP_pos == 1,
      "Positivo",
      "Negativo"
    ),
  sympt_gastrointestinal = if_else(
      ffi_is_symp_month_nausea == 1 |
      ffi_is_symp_month_vomit == 1 |
      ffi_is_symp_month_stomach == 1,
      "Sí", "No"
    ),
  sympt_constitucional = if_else(
      ffi_is_fever_month == 1 |
      ffi_is_symp_month_chills == 1 |
      ffi_is_symp_month_heavysweat == 1 |
      ffi_is_symp_month_general == 1,
      "Sí", "No"
    ),
  sympt_neurologico = if_else(
      ffi_is_symp_month_head == 1 |
      ffi_is_symp_month_dizz == 1,
      "Sí", "No"
    ),
  Chikungunya_Pos = if_else(
    Chik.E1_pos == 1, "Positivo", "Negativo"
    ),
  Zika_Pos = if_else(
    Zika.NS1_pos == 1, "Positivo", "Negativo"
    ),
  Vivax_Pos = if_else(
    pv_exposure == 1, "Positivo", "Negativo"
    ),
   Falciparum_Pos = if_else(
    pf_exposure == 1, "Positivo", "Negativo"
    ),
   economic_activities = factor(ffi_is_main_econ_act,
                                 labels = c("None",
                                            "Day labourer",
                                            "Wood extractor",
                                            "Fisherman",
                                            "Livestock farmer",
                                            "Farmer",
                                            "Trader",
                                            "Housewife",
                                            "Student",
                                            "Motorcycle taxi driver",
                                            "Other")
    ),
   EconomicActivities_new = case_when(
      economic_activities == "Fisherman" ~ "Fisherman",
      economic_activities == "Farmer" ~ "Farmer",
      economic_activities == "Housewife" ~ "Housewife",
       economic_activities == "Student" ~ "Student",
      economic_activities %in% c("Day labourer", "Trader", "Motorcycle taxi driver") ~ "Services/Commerce",
      economic_activities %in% c("Other", "Wood extractor", "Livestock farmer") ~ "Other",
      economic_activities == "None" ~ "None",
      TRUE ~ "Other"
    )
  ) %>%
  mutate(
    EconomicActivities_new = factor(
      EconomicActivities_new,
      levels = c("Fisherman", "Farmer", "Housewife", "Student",
                 "Services/Commerce", "Other", "None")
    ))
  
```

Paso 2: Creacion de la tabla

```{r}
individuals <- ffi_hh_ind_seropos %>%
  select(
    ffi_is_district,
    ffi_is_health_facility_name,
    ffi_is_community,
    ffi_h_code,
    gender,
    age_oms,
    time_resi_cat2,
    education_summary,
    sympt_constitucional,
    sympt_osteomuscular,
    sympt_gastrointestinal,
    sympt_neurologico,
    Schistosomiasis,
    Chikungunya_Pos,
    Zika_Pos,
    pv_exposure,
    pf_exposure,
    distance_to_rh_hf_sampl,
    EconomicActivities_new,
    economic_activities,
    ffi_is_fever_month,
    ffi_is_symp_month_chills,
    ffi_is_symp_month_heavysweat,
    ffi_is_symp_month_general) 

tab1_df <- individuals %>% 
  select(-c(ffi_is_health_facility_name,
    ffi_is_community,
    ffi_h_code)
    )

df_multinivel <- individuals %>% 
  select(-c(
    Schistosomiasis,
    Chikungunya_Pos,
    Zika_Pos,
    pv_exposure,
    pf_exposure
  )
  )

tab1_df %>%
  tbl_summary(by = ffi_is_district, missing = "no") %>%
  add_overall() %>%
  add_p() %>%
  bold_labels()

#write_csv(df_multinivel, file = "./data/mid/individuals_multilevel_df.csv")
```

# Tabla 2

```{r}

#Paquetes estadisticos

library(dplyr)
library(gtsummary)

#Tabla de Schistosoma

ffi_hh_ind_seropos %>%
  select(
    ffi_is_district,
    gender,
    age_oms,
    time_resi_cat2,
    education_summary,
    sympt_constitucional,
    sympt_osteomuscular,
    sympt_gastrointestinal,
    sympt_neurologico,
    distance_to_rh_hf_sampl,
    ffi_is_trip_month,
    EconomicActivities_new,
    ffi_is_fever_month,
    ffi_is_symp_month_chills,
    ffi_is_symp_month_heavysweat,
    ffi_is_symp_month_general,
    Schistosomiasis
  ) %>%
  tbl_summary(
    by = Schistosomiasis,
    missing = "ifany",
    label = list(
      ffi_is_district ~ "District",
      gender ~ "Gender",
      age_oms ~ "Age group",
      time_resi_cat2 ~ "Residence time",
      education_summary ~ "Education level",
      sympt_constitucional ~ "Constitutional symptoms",
      sympt_osteomuscular ~ "Osteomuscular symptoms",
      sympt_gastrointestinal ~ "Gastrointestinal symptoms",
      sympt_neurologico ~ "Neurological symptoms",
      distance_to_rh_hf_sampl ~ "Distance to Health Facility (sample)",
      ffi_is_trip_month ~ "Travel in last month"
    )
  ) %>%
  add_overall() %>%
  add_p() %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Schistosomiasis status**")

#Tabla de Zika
ffi_hh_ind_seropos %>%
  select(
    ffi_is_district,
    gender,
    age_oms,
    time_resi_cat2,
    education_summary,
    sympt_constitucional,
    sympt_osteomuscular,
    sympt_gastrointestinal,
    sympt_neurologico,
    distance_to_rh_hf_sampl,
    ffi_is_trip_month,
    EconomicActivities_new,
    ffi_is_fever_month,
    ffi_is_symp_month_chills,
    ffi_is_symp_month_heavysweat,
    ffi_is_symp_month_general,
    Zika_Pos
  ) %>%
  tbl_summary(
    by = Zika_Pos,
    missing = "ifany",
    label = list(
      ffi_is_district ~ "District",
      gender ~ "Gender",
      age_oms ~ "Age group",
      time_resi_cat2 ~ "Residence time",
      education_summary ~ "Education level",
      sympt_constitucional ~ "Constitutional symptoms",
      sympt_osteomuscular ~ "Osteomuscular symptoms",
      sympt_gastrointestinal ~ "Gastrointestinal symptoms",
      sympt_neurologico ~ "Neurological symptoms",
      distance_to_rh_hf_sampl ~ "Distance to Health Facility (sample)",
      ffi_is_trip_month ~ "Travel in last month"
    )
  ) %>%
  add_overall() %>%
  add_p() %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Zika status**")

#Tabla de Chikungunya
ffi_hh_ind_seropos %>%
  select(
    ffi_is_district,
    gender,
    age_oms,
    time_resi_cat2,
    education_summary,
    sympt_constitucional,
    sympt_osteomuscular,
    sympt_gastrointestinal,
    sympt_neurologico,
    distance_to_rh_hf_sampl,
    ffi_is_trip_month,
    EconomicActivities_new,
    ffi_is_fever_month,
    ffi_is_symp_month_chills,
    ffi_is_symp_month_heavysweat,
    ffi_is_symp_month_general,
    Chikungunya_Pos
  ) %>%
  tbl_summary(
    by = Chikungunya_Pos,
    missing = "ifany",
    label = list(
      ffi_is_district ~ "District",
      gender ~ "Gender",
      age_oms ~ "Age group",
      time_resi_cat2 ~ "Residence time",
      education_summary ~ "Education level",
      sympt_constitucional ~ "Constitutional symptoms",
      sympt_osteomuscular ~ "Osteomuscular symptoms",
      sympt_gastrointestinal ~ "Gastrointestinal symptoms",
      sympt_neurologico ~ "Neurological symptoms",
      distance_to_rh_hf_sampl ~ "Distance to Health Facility (sample)",
      ffi_is_trip_month ~ "Travel in last month"
    )
  ) %>%
  add_overall() %>%
  add_p() %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Chikungunya status**")


#Tabla de Vivax
ffi_hh_ind_seropos %>%
  select(
    pv_exposure,
    ffi_is_district,
    gender,
    age_oms,
    time_resi_cat2,
    education_summary,
    sympt_constitucional,
    sympt_osteomuscular,
    sympt_gastrointestinal,
    sympt_neurologico,
    distance_to_rh_hf_sampl,
    ffi_is_trip_month,
    ffi_is_fever_month,
    ffi_is_symp_month_chills,
    ffi_is_symp_month_heavysweat,
    ffi_is_symp_month_general,
    EconomicActivities_new
  ) %>%
  tbl_summary(
    by = pv_exposure,
    missing = "ifany",
    label = list(
      ffi_is_district ~ "District",
      gender ~ "Gender",
      age_oms ~ "Age group",
      time_resi_cat2 ~ "Residence time",
      education_summary ~ "Education level",
      sympt_constitucional ~ "Constitutional symptoms",
      sympt_osteomuscular ~ "Osteomuscular symptoms",
      sympt_gastrointestinal ~ "Gastrointestinal symptoms",
      sympt_neurologico ~ "Neurological symptoms",
      distance_to_rh_hf_sampl ~ "Distance to Health Facility (sample)",
      ffi_is_trip_month ~ "Travel in last month"
    )
  ) %>%
  add_overall() %>%
  add_p() %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Plasmodium Vivax**")

#Tabla de Falciparum
ffi_hh_ind_seropos %>%
  select(
    pf_exposure,
    ffi_is_district,
    gender,
    age_oms,
    time_resi_cat2,
    education_summary,
    sympt_constitucional,
    sympt_osteomuscular,
    sympt_gastrointestinal,
    sympt_neurologico,
    distance_to_rh_hf_sampl,
    ffi_is_trip_month,
    ffi_is_fever_month,
    ffi_is_symp_month_chills,
    ffi_is_symp_month_heavysweat,
    ffi_is_symp_month_general,
    EconomicActivities_new
  ) %>%
  tbl_summary(
    by = pf_exposure,
    missing = "ifany",
    label = list(
      ffi_is_district ~ "District",
      gender ~ "Gender",
      age_oms ~ "Age group",
      time_resi_cat2 ~ "Residence time",
      education_summary ~ "Education level",
      sympt_constitucional ~ "Constitutional symptoms",
      sympt_osteomuscular ~ "Osteomuscular symptoms",
      sympt_gastrointestinal ~ "Gastrointestinal symptoms",
      sympt_neurologico ~ "Neurological symptoms",
      distance_to_rh_hf_sampl ~ "Distance to Health Facility (sample)",
      ffi_is_trip_month ~ "Travel in last month"
    )
  ) %>%
  add_overall() %>%
  add_p() %>%
  bold_labels() %>%
  modify_header(label = "**Variable**") %>%
  modify_spanning_header(c("stat_1", "stat_2") ~ "**Plasmodium Falciparum**")

```
