...
# Data Preparation
df_bin <- ffi %>% 
  filter(distance_to_rh_hf_sampl=="Proximate") %>% 
  mutate(
    Schistosomiasis = if_else(
      SEA_pos == 1 | X229.E.NP_pos == 1,
      "Si",
...
b <- ggraph(grafo, layout = layout) +
  ...
  theme_void() +
  labs(
    title = "B. Proximate to the main HF",
    size = "N° of seropositivity",
    edge_width = "N° of connections"
  ) +
  guides(
...
#--------------------------------------------
df_bin <- ffi %>% 
  filter(distance_to_rh_hf_sampl=="Moderate") %>% 
  mutate(
    Schistosomiasis = if_else(
      SEA_pos == 1 | X229.E.NP_pos == 1,
      "Si",
...
c <- ggraph(grafo, layout = layout) +
  ...
  theme_void() +
  labs(
    title = "C. Moderate distance to the main HF",
    size = "N° of seropositivity",
    edge_width = "N° of connections"
  ) +
  guides(
...
#-------------------------------------------
df_bin <- ffi %>% 
  filter(distance_to_rh_hf_sampl=="Distant") %>% 
  mutate(
    Schistosomiasis = if_else(
      SEA_pos == 1 | X229.E.NP_pos == 1,
      "Si",
...
d <- ggraph(grafo, layout = layout) +
  ...
  theme_void() +
  labs(
    title = "D. Distant to the main HF",
    size = "N° of seropositivity",
    edge_width = "N° of connections"
  ) +
  guides(
...
#----------------------------------------------
df_bin <- ffi %>% 
  filter(distance_to_rh_hf_sampl=="Extra Distant") %>% 
  mutate(
    Schistosomiasis = if_else(
      SEA_pos == 1 | X229.E.NP_pos == 1,
      "Si",
...
e <- ggraph(grafo, layout = layout) +
  ...
  theme_void() +
  labs(
    title = "E. Extra distant to the main HF",
    size = "N° of seropositivity",
    edge_width = "N° of connections"
  ) +
  guides(
...
library(ggpubr)
b1 <- ggarrange(b,c,d,e, ncol = 2, nrow =2)
a1 <- ggarrange(a, b1, ncol = 2, nrow =1)
ggsave("output/network_distance.png", plot = a1, 
       dpi = 600, height = 8, width = 18.5, bg = "white")
